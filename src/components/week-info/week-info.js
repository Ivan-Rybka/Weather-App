...
export default class WeekInfo extends Component {
    render () {
        const data = Object.values(this.props.week).slice(1, 7)
        return (
            <div id="content-3">
                {
                    data?.map((item) => {
                        ...
                        return (
                            <div key={item.day+item.wind*item.pressure} className="weekItem">
                                <h4>{item.day}</h4>
                                <img src={weatherIcon} alt=""/>
                                <div className="weekItem_info">
                                    <p>Day: {item.dayTemp}&#8451;</p>
                                    <p>Night: {item.nightTemp}&#8451;</p>
                                    <p>Wind: {item.wind}m/s</p>
                                    <p>Pressure: {item.pressure}mmHg</p>
